...
for instrument in instrumentlist:
  ...
  for code in diag_conv.code[indx]:
    if code not in reptypelist:
      reptypelist.append(code)

#  reptypelist =[220,221,224,229,230,280,282,290]
...
#   if reptype == 720:
#     continue
#   if reptype > 240 and reptype < 260:
#     continue

    if reptype == 220 or reptype == 221 or reptype == 232: 
      color='b'
      platformname='ADPUPA('+str(reptype)+')'
    elif reptype == 224: 
      color='darkorange'
      platformname='VADWND('+str(reptype)+')'
    elif reptype == 230: 
      color='g'
      platformname='AIRCFT('+str(reptype)+')'
    elif reptype == 229: 
      color='c'
      platformname='PROFLR('+str(reptype)+')'
    elif reptype == 280 or reptype == 282: 
      color='r'
      platformname='SFCSHP('+str(reptype)+')'
    elif reptype == 290: 
      color='olive'
      platformname='ASCATW('+str(reptype)+')'
    else: 
      color='k'
      print(diag_conv.subcode)
  ...
  plt.savefig('V'+'_'+str(dtg)+'.png',dpi=100)
  plt.show()
